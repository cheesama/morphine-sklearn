...
def convert_ner_data_format(line, tokenize_fn=tokenize_fn):
    ...
    text = re.sub(r"\([^)]*\)", "", line).replace("[", "").replace("]", "")
    ...
    while start_position >= 0:
        entity_type = line[
            line.find("]", start_position) + 1 + 1 : line.find(")", start_position + 1)
        ]
        entity_value = line[start_position + 1 : line.find("]", start_position + 1)]
        entity_position = (
            text.find(entity_value),
            text.find(entity_value) + len(entity_value),
        )
        ...
        # print (f"type: {entity_types}\tvalue: {entity_values}\tposition: {entity_positions}")

        start_position = line.find("[", start_position + 1)
